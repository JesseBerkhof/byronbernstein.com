@if ($paginator->hasPages())
    <div class="flex items-centergray-600">
        {{-- Previous Page Link --}}
        @if ($paginator->onFirstPage())
            <span class="rounded-l rounded-sm border border-gray-900 px-3 py-2 cursor-not-allowed no-underline bg-gray-800">&laquo;</span>
        @else
            <a
        ...
        @foreach ($elements as $element)
            ...
            @if (is_string($element))
                <span class="border-t border-b border-l border-gray-900 px-3 py-2 cursor-not-allowed no-underline bg-gray-800">{{ $element }}</span>
            @endif
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <span class="border-t border-b border-l border-gray-900 px-3 py-2 bg-gray-700 no-underline bg-gray-800">{{ $page }}</span>
                    @else
